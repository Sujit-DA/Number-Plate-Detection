...
COOLDOWN_PERIOD = timedelta(seconds=200)
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                if label:
                    ...
                    cursor = conn.cursor()
                    cursor.execute('''
                        SELECT status, exit_time FROM LicensePlates WHERE license_plate = ?
                    ''', (label,))
                    record = cursor.fetchone()
                    conn.close()
                    ...
                    if record and record[0] == 'In':
                        ...
                        generate_invoice(label)  # Generate and print the bill
                    elif record and record[0] == 'Out':
                        # Check if cooldown period has passed
                        exit_time_dt = datetime.fromisoformat(record[1])
                        if current_time - exit_time_dt >= COOLDOWN_PERIOD:
                            # Create a new entry for the vehicle
